...
(
    ...
    ProductId INT NOT NULL
        FOREIGN KEY REFERENCES StoreApp.Product (ProductId) ON DELETE CASCADE ON UPDATE CASCADE,
        PRIMARY KEY (LocationId, CustomerId, ProductId),
    OrderQuantity INT NOT NULL CHECK ( OrderQuantity <= 50),
...
INSERT INTO StoreApp.Orders (LocationId, CustomerId, ProductId, OrderQuantity) VALUES 
((SELECT LocationId FROM StoreApp.Location WHERE Address = '820 Ashley Lane'), (SELECT CustomerId FROM StoreApp.Customer WHERE Email = 'martink@outlook.com'), (SELECT ProductId FROM StoreApp.Product WHERE  Name = 'Samsung - Galaxy Tab'), 3),
((SELECT LocationId FROM StoreApp.Location WHERE Address = '44 Colonial Drive'), (SELECT CustomerId FROM StoreApp.Customer WHERE Email = 'martink@outlook.com'), (SELECT ProductId FROM StoreApp.Product WHERE  Name = 'Samsung - Galaxy Tab'), 5),
((SELECT LocationId FROM StoreApp.Location WHERE Address = '820 Ashley Lane'), (SELECT CustomerId FROM StoreApp.Customer WHERE Email = 'jfinke@live.com'), (SELECT ProductId FROM StoreApp.Product WHERE  Name = 'Surface Pro X'), 7),
((SELECT LocationId FROM StoreApp.Location WHERE Address = '86 Winding Way St'), (SELECT CustomerId FROM StoreApp.Customer WHERE Email = 'amichalo@verizon.net'), (SELECT ProductId FROM StoreApp.Product WHERE  Name = 'Insignia Smart Fire TV'), 11);
